...
import { Plus, Building2, Users, CreditCard, Edit, Trash2, UserPlus, Eye } from "lucide-react";
...
export default function SuperAdminDashboard() {
  const { clinics, plans, fetchClinics } = useAuthStore();
  ...
  const handleCreateUser = async () => {
    ...
    if (error) { toast({ title: "Gabim", description: error.message, variant: "destructive" }); }
    else { toast({ title: "Përdoruesi u krijua!" }); setUserDialogOpen(false); setUserForm({ email: "", password: "", full_name: "", role: "clinic_admin", clinic_id: "" }); }
  };
  ...
  const getPlanForClinic = (planId: string) => plans.find((p) => p.id === planId);
    ...
    <div className="p-6 space-y-6 max-w-7xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Menaxho klinikat, planet dhe përdoruesit</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="rounded-card bg-card shadow-subtle p-4">
          <div className="flex items-center gap-2 text-muted-foreground mb-2"><Users className="h-4 w-4" /><span className="text-xs">Aktive</span></div>
          <p className="text-2xl font-semibold">{clinics.filter((c) => c.is_active).length}</p>
        </div>
      </div>
      ...
      {/* Clinics */}
      <div>
        <h2 className="text-sm font-semibold mb-3">Klinikat ({clinics.length})</h2>
